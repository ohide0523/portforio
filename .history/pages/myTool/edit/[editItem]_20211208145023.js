...
const editItem = () => {
  ...
  const { uid,set } = useContext(Context);
...
const onClickDelete_item = ()=>{
  ...
  db.collection("users").doc(uid).collection("items").doc(editItem).delete()
  // db.collection("users").doc(uid).collection("items").get().then((snapshot)=>{
  ...
  return (
    <>
      ...
      >
        ...
        <Link href="/Top">
          <Button variant="contained"　onClick={onClickUpdate_item} sx={{width:"100%",marginTop:"10px"}}>更新</Button>
        </Link>
       
        <Button variant="contained"　onClick={onClickDelete_item} sx={{width:"100%",marginTop:"50px",background:"red"}}>削除</Button>
