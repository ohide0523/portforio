...
const editItem = () => {
  ...
  useEffect(()=>{
    if(uid && editItem){
        ...
        db.collection("users").doc(uid).collection("items").doc(editItem).get().then((doc)=>{
  ...
  const onClickUpdate_item = () => {
    db.collection("users").doc(uid).collection("items").doc(editItem).upda(
      {
  ...
  return (
   <>
      ...
      >
        ...
        <Link href="/Top">
         <Button onClick={onClickUpdate_item}>更新</Button>
        </Link>
      </div>
    </>
  );
