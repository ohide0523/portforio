...
const item = () => {
  ...
  const { uid,another } = useContext(Context);
  ...
  const onClickAdd = () => {
    ...
    db.collectionGroup("items").orderBy("createAt","desc").get().then((snapshot)=>{
      snapshot.forEach((doc)=>{
        setItems(prev=>[...prev,doc.da])
      })
    })
    alert("募集しました！");
  };
  ...
  return (
    <>
      <h1 style={{ textAlign: "center" }}>募集入力ページ</h1>
      <div style={{ textAlign: "center" }}>
        {previewImg && (
          <img src={itemURL} style={{ width: "40%", height: "40%" }} onClick={onClickPhotoDelete}/>
        )}
      </div>

      <div
       className={Styles.create_item_container}
      >
