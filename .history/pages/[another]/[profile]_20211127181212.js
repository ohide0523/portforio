...
const [follow,setFollow] = useState(false)
  const { uid, anotherUser,getAnotherUser} =
    useContext(Context);
...
// フォローボタンの状態の取得
const getFollow =()=>{
    db.collection("users").doc(uid).collection("followLists").where("followUser","==",another).onSnapshot((snapshot)=>{
      ...
      <h1>他人のプロフィール</h1>
        ...
        {anotherUser.length > 0 &&
          anotherUser.map((user, index) => (
            <div>
            <di key={index}  style={{display:"flex",alignItems:"center",justifyContent:"center"}}>
                <Avatar
                alt="アバター画像"
                src={user.img}
