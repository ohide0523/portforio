...
const mypage = () => {
  const { likedItems, followLists, myUser,myUserItems,uid } = useContext(Context);
      ...
      <div style={{marginBottom:"100px"}}>
        {followLists.length > 0 &&
          followLists.map((list, index) => (
              ...
              <div style={{display:"flex",justifyContent:"center",alignItems:"centerflexDirection:"column"}}>
              <Avatar
                alt="アバター画像"
                src={list.img}
              ...
              />
              <div style={{display:"flex",flexDirection:"column",marginLeft:"20px"}}>
                     <div>ニックネーム:{list.name}</div>
              <div>メール:{list.email}</div>
              <div>フォロワー数:{list.follower}</div>
              </div>
              </div>
