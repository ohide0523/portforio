import Link from "next/link";
import { useRouter } from "next/router";
import React, { useContext, useEffect } from "react";
import { Context } from "../../components/Context";

import ImageList from "@mui/material/ImageList";
import ImageListItem from "@mui/material/ImageListItem";
import ImageListItemBar from "@mui/material/ImageListItemBar";
import Tooltip from "@mui/material/Tooltip";
import ArrowForwardIosIcon from "@mui/icons-material/ArrowForwardIos";
import Button from "@mui/material/Button";
import Avatar from "@mui/material/Avatar";

import Styles from "../../styles/Home.module.css";
import { makeStyles } from "@material-ui/core";

const useStyles = makeStyles((theme) => ({
  root: {
    width: "90%",
    minHeight: "300px",
  },
}));

const mypage = () => {
  const { likedItems, followLists, myUser, myUserItems, uid } =
    useContext(Context);
  const router = useRouter();
  const classes = useStyles();

  // useEffect(() => {
  //   if (uid == null) {
  //     router.push("/Login");
  //   }
  // }, [uid]);

  //いいねした全itemのページに遷移する関数
  const onClickAllLikedItems = () => {
    router.push("/myTool/myFavorite/favorite");
  };

  return (
    <div>
      <div
        style={{
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
        }}
      >
        <h1 className={Styles.mypage_h1}>マイページ</h1>
      </div>

      {myUser.length > 0 &&
        myUser.map((user, index) => (
          <div key={index}>
            <div className={Styles.profile_top}>
              <Avatar
                alt="アバター画像"
                src={user.img}
                className={Styles.profile_avatar}
              />
              <div
                style={{
                  display: "flex",
                  flexDirection: "column",
                  marginLeft: "20px",
                  fontSize: "15px",
                  lineHeight: "1.5",
                }}
              >
                <div>ニックネーム:{user.name}</div>
                <div>メール:{user.email}</div>
                <div>フォロワー数:{user.follower}</div>
                <Link href="/myTool/edit/editProfile">
                  <Button
                    variant="outlined"
                    component="div"
                    sx={{ marginTop: "10px" }}
                  >
                    プロフィールを編集する
                  </Button>
                </Link>
              </div>
            </div>
            <div
             className={Styles.myUser_produce}
            >
              <h2>自己紹介</h2>
              <div>{user.introduce}</div>
            </div>
          </div>
        ))}

      <div style={{ marginTop: "100px", marginBottom:"50px"}}>
        <h2 className={Styles.profile_h2}>
          自分が里親を募集したワンちゃんたち
        </h2>
        <hr />

        <div >
          <ImageList cols={3} className={Styles.item_list}>
            {myUserItems.length > 0 &&
              myUserItems.map((item, index) => (
                <Link
                  key={index}
                  href="/[another]/anotherItem/[item]"
                  as={`/${item.userId}/anotherItem/${item.id}`}
                >
                  <ImageListItem key={item.img}>
                    <img
                      src={item.img}
                      srcSet={item.img}
                      alt={item.title}
                      loading="lazy"
                      
                    />

                    <ImageListItemBar
                      title={item.title}
                      subtitle={item.author}
                      className={Styles.item_list_bar}
                    />
                  </ImageListItem>
                </Link>
              ))}
          </ImageList>
        </div>
      </div>



      <div style={{ marginTop: "50px",}}>
      <h2 style={{ lineHeight: "1px" }}>いいねしたワンちゃんたち</h2>
      <hr />

      <div style={{ display: "flex", width:"100%",marginBottom:"40px"}}>
        <ImageList cols={3} className={Styles.item_list}>
          {likedItems.length > 0 &&
            likedItems.map((item, index) => (
              <Link
                key={index}
                href="/[another]/anotherItem/[item]"
                as={`/${item.userId}/anotherItem/${item.id}`}
              >
                <ImageListItem>
                  <img
                    src={item.img}
                    srcSet={item.img}
                    alt={item.title}
                    loading="lazy"
                    style={{ position: "relative" }}
                  />

                  <ImageListItemBar title={item.title} subtitle={item.category} className={Styles.item_list_bar}/>
                </ImageListItem>
              </Link>
            ))}
        </ImageList>
        {likedItems.length > 2 && (
          <Tooltip title="もっと見る" arrow>
            <ArrowForwardIosIcon
              className={Styles.list_item_arrow}
              onClick={onClickAllLikedItems}
            />
          </Tooltip>
        )}
      </div>
      </div>

      <h2 style={{ lineHeight: "1px" }}>フォローしているユーザー</h2>
      <hr />

      <div className={Styles.follow_list_container}>
        {followLists.length > 0 &&
          followLists.map((list, index) => (
            <div
              className={Styles.follow_list_item_container}
              key={index}
            >
              <div className={Styles.follow_list_item}>
                <Link
                  href="/[another]/[profile]"
                  as={`/${list.followUser}/profile`}
                  key={index}
                >
                  <Avatar
                    alt="アバター画像"
                    src={list.img}
                    className={Styles.follow_list_item_icon}
                  />
                </Link>
                <div
                 className={Styles.follow_list_item_text}
                >
                  <div className={Styles.follow_list_item_user_name}>{list.name}</div>
                  <div className={Styles.follow_list_item_user_follower}>{list.followerCount}フォロワー</div>
                </div>
              </div>
            </div>
          ))}
      </div>
    </div>
  );
};

export default mypage;
