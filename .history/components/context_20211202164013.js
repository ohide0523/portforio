...
const ContextProvider = ({ children }) => {
  ...
  const getItems = async() => {
    const initQuery = db.collectionGroup("items").orderby("createAt","desc").limit(30);
    const initData = await initQuery.get()

    const lastData = getLastData(initData)
    const query = db.collectionGroup("items").orderBy("createAt","desc").endBefore(lastData.createAt)
    
         db.collection("items").orderBy("createAt","desc").onSnapshot((snapshot)=>{
      snapshot.docChanges().forEach(change=>{
  ...
  const getAnotherUser = () => {
    db.collection("users")
      .doc(another)
      .onSnapshot((doc) => {
        setAnotherUser(prev=>[...prev,doc.data()]);
      });
  };
