...
const ContextProvider = ({ children }) => {
  ...
  //各itemのlikeの状態を取得
  const getLike = () => {
    db.collection("users").doc(uid).collection("likedItems").where("id","==",item).get().then((snapshot)=>{
  ...
  //全ユーザーのitemsの取得
  const getItems = () => {
    db.collectionGroup("items").limit(30).onSnapshot((snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === "added") {
  ...
  // 自分の出したitemの取得
  db.collection("users").doc(uid).collection("items").onSnapshot((snapshot)=>{
    snapshot.docChanges().forEach((change)=>{
      if(change.type==="added"){
